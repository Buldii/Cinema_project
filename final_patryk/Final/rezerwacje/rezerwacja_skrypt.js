const columns = 15;
const rows = 10;
const sesja_klienta = document.getElementById("user").innerText;
...
console.log(JSON.stringify({ user: sesja_klienta, godzina: id_godzina }));
async function drawSeats() {
  ...
  const response = await fetch("./get_seats.php", {
    ...
    method: "POST",
    body: JSON.stringify({ user: sesja_klienta, godzina: id_godzina }),
  });
